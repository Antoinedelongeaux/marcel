...
export async function getMemories_Question(
  subject_active,
  setQuestion,
  setAnswers,
  setOwner,
  ...
  try {
    ...
    let query = supabase.from("Memoires_questions").select("*");
    ...
    // Exécuter la requête
    const { data: questionsWithTags, error: errorQuestionsWithTags } =
      await query;
    ...
    const {
      data: answersForSelectedQuestion,
      error: errorAnswersForSelectedQuestion,
    } = await supabase
...
export async function getMemories_Question_by_id(
  id_question,
  setQuestion,
  setAnswers,
  setOwner
...
export async function getMemories_Questions(
  subject_active,
  setQuestions,
  tags,
  personal
...
export async function getMemories_Questions_Published(
  subject_active,
  setQuestions,
  tags,
  personal
...
export const delete_audio = async (name) => {
  const { data, error } = await supabase.storage.from("audio").remove([name]);
};
...
export async function submitMemories_Answer(
  answer,
  question,
  session,
  audio,
  ...
  try {
    ...
    const { error } = await supabase.from("Memoires_answers").insert([
      {
    ...
    Alert.alert(
      audio ? "Réponse audio enregistrée" : "Réponse écrite enregistrée"
    );
    resetAnswerAndFetchQuestion();
  } catch (error) {
...
export async function submitMemories_Answer_written(
  answer,
  ID_USER,
  Id_question,
  Id_connection,
    ...
    const { error } = await supabase.from("Memoires_answers").insert([
      {
        ...
        rank: rank,
        question_reponse: question_reponse,
        id_answer_source: id_answer_source,
      },
    ]);
...
export async function submitMemories_Answer_oral(
  answer,
  ID_USER,
  Id_question,
  Id_connection,
...
export async function submitMemories_Answer_image(
  answer,
  ID_USER,
  Id_question,
  Id_connection,
...
export async function save_question(
  question,
  tags,
  subject_active,
  setMiscState
) {
  const id_question = [...Array(12)]
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      const { error } = await supabase.from("Memoires_questions").insert([
        {
          id: id_question,
          id_subject: subject_active,
          question: question,
          tags: tags,
        },
      ]);
...
export async function listSubjects(id_user) {
  try {
    ...
    const associatedSubjectIds = associatedSubjects.map(
...
export async function joinSubject(
  id_subject,
  id_user,
  access,
  Inspirer,
  ...
  try {
    ...
    if (existing) {
      console.log(
        "Vous avez déjà émis une demande d'accès à ce projet. Le propriétaire doit valider votre demande"
      );
      ...
      .insert([
        {
          ...
          Publier,
          Lire,
        }, // Assurez-vous d'inclure id_user dans l'insertion
      ]);
    ...
    console.error(
      "Erreur lors de la tentative d'ajout du contributeur :",
      error.message
    );
...
export async function getSubjects(id_user) {
  try {
    ...
    const subjectsWithContent = await Promise.all(
      list_subjects.map(async (element) => {
...
export async function create_project(name, id_user, option) {
  // Exemple de pseudo code, à adapter selon votre logique d'application
  try {
    const id = customUUIDv4();

    if (option == "structurer_projet") {
      const { data, error } = await supabase
        .from("Memoires_subjects") // Remplacez 'projects' par le nom de votre table
        .insert([
          {
            id: id,
            title: name,
            etape_collecting: false,
            etape_writting: true,
            etape_publishing: false,
          },
        ]);
    } else {
      const { data, error } = await supabase
        ...
        .insert([
          {
            id: id,
            title: name,
            etape_collecting: true,
            etape_writting: false,
            etape_publishing: false,
          },
        ]);
    }
...
export async function create_chapter(title, id_subject) {
  ...
  try {
    const { data, error } = await supabase
      ...
      .insert([{ title: title, id_subject: id_subject }]);
...
export async function validate_project_contributors(
  id_subject,
  id_user,
  access,
  Inspirer,
...
export async function updateSubject(id_subject, column_to_update, value) {
  try {
    const updates = {};
...
export async function getUserStatus(id_user, id_subject) {
  try {
    ...
    Alert.alert(
      "Erreur lors de la prise en compte de la copie : ",
      error.message
    );
    return "non trouvé";
  }
...
export async function linkAnalysis(suffix) {
  try {
    ...
    if (data_subject) {
      return {
        nature: "subject",
        id_subject: data_subject.id_subject,
        Inspirer: data_subject.Inspirer,
        Raconter: data_subject.Raconter,
        Réagir: data_subject.Réagir,
        Structurer: data_subject.Structurer,
        Rédiger: data_subject.Rédiger,
        Relire: data_subject.Relire,
        Publier: data_subject.Publier,
        Lire: data_subject.Lire,
      };
    }
...
export async function createNewLink(id_target, id_question_id_subject) {
  try {
    let link;
    const insertData =
      id_question_id_subject === "id_question"
...
export async function createTheme(theme, id_subject) {
  try {
    ...
    const { error } = await supabase
      ...
      .insert([{ theme: theme, id: id, id_subject: id_subject }]);
