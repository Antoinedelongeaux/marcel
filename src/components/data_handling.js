...
export async function getMemories_Question(subject_active, setQuestion, setAnswers, setOwner, index, tags, personal) {
  try {
    ...
    // Vérifier si l'index spécifié est dans la plage des questions récupérées
    if (index < 0 || index >= questionsWithTags.length) {
      setQuestion({ "question": "End" });
      setAnswers([]);
      return "Plus de question";
    }
...
export async function save_question(question, tags, subject_active) {
