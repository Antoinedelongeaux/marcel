...
const OrientationScreen = ({ route }) => {
  ...
  const details = [
    <>
      <Text style={{ fontWeight: "bold" }}>Proposez</Text> un thème autour
      duquel l'information sera collectée
    </>,
    <>
    ...
    <>
      Faites vos <Text style={{ fontWeight: "bold" }}>commentaires</Text>, poser
      vos <Text style={{ fontWeight: "bold" }}>questions</Text>, concernant qui
      a été raconté. {"\n\n"} Ou bien {"\n\n"}{" "}
      <Text style={{ fontWeight: "bold" }}>répondez</Text> à des questions déjà
      posées.
    </>,
    <>
      <Text style={{ fontWeight: "bold" }}>Définissez</Text> les parties de
      l'ouvrage ainsi que leurs chapitres
    </>,
    <>
      <Text style={{ fontWeight: "bold" }}>Rédigez</Text> un chapitre à partir
      de la matière déjà collectée
    </>,
    <>
      <Text style={{ fontWeight: "bold" }}>Relisez</Text>, complétez, annotez un
      chapitre rédigé
    </>,
    <>
      <Text style={{ fontWeight: "bold" }}>Définissez</Text> quels chapitres
      sont prêts à être publier, et comment ils vont l'être
    </>,
    <>
  ...
  const handleChoice_2 = async (selectedSubject) => {
    if (selectedSubject) {
      if (!selectedSubject.etape_collecting) {
        if (selectedSubject.etape_writting) {
          navigateToScreen("Marcel", {
            session: session,
            initialStatut: "Structurer",
          });
        } else {
          if (selectedSubject.etape_publishing) {
            navigateToScreen("Marcel", {
              session: session,
    ...
    <View style={globalStyles.container} ref={containerRef}>
              ...
              <TouchableOpacity
                ...
                onPress={async () => {
                  try {
                    ...
                    // Attendre que l'ID du sujet soit enregistré
                    await saveActiveSubjectId(selectedSubject.id);

                    // Ensuite, mémoriser le sujet actif
                    await remember_active_subject(
                      selectedSubject.id,
                      session.user.id
                    );

                    // Mettre à jour le sujet sélectionné
                    setSubject(selectedSubject);

                    // Appeler la fonction de gestion du choix
                    handleChoice_2(selectedSubject);
                  } catch (error) {
      ...
      {progressiveMessage_2 !== "" && (
        <>
          ...
          {isLargeScreen ? (
            <View style={styles.container_fullScreen}>{renderCards()}</View>
          ) : (
