...
const useFetchData = (
  ...
  useEffect(() => {
    ...
    const hasChanged = !deepEqual(
      previousValues.current.userStatus,
      userStatus
    );
    ...
    if (hasChanged) {
      ...
      if (subjectActive) {
        if (statut === "Lire" && subject.etape_publishing) {
          getMemories_Questions_Published(
            subjectActive,
            setQuestions,
            tags,
...
function ReadAnswersScreen({ route }) {
  ...
  const actOnStatut = async () => {
    ...
    if (statut && statut === "Raconter") {
      ...
      if (themes.length === 0) {
        setTextBlockage(
          "Vous devez définir au moins un thème (via l'onglet 'Inspirer') avant d'en parler."
        );
        setIsBlocage(true);
      } else {
        setIsBlocage(false);
    ...
    if (statut && statut === "Réagir") {
      if (themesAllUsers.length === 0) {
        setTextBlockage(
          "Il n'y a pour l'instant aucun contenu (ajouté via les onglets 'Inspirer' et 'Raconter') sur lequel réagir."
        );
        setIsBlocage(true);
      } else {
        setIsBlocage(false);
    ...
    if (statut && statut === "Rédiger") {
      if (questions.length === 0) {
        setTextBlockage(
          "Vous devez définir au moins un chapitre (via l'onglet 'Structurer') avant de le rédiger."
        );
        setIsBlocage(true);
      } else {
        setIsBlocage(false);
    ...
    if (statut && statut === "Publier") {
      if (questions.length === 0) {
        setTextBlockage(
          "Il n'y a pas encore de chapitre à publier dans ce projet."
        );
        setIsBlocage(true);
      } else {
        setIsBlocage(false);
    ...
    if (statut && statut === "Lire") {
      const hasPrivatePublishedQuestion = questions.some(
        (question) => question.published_private === true
      );
...
*/
  ...
  const handleAssociateQuestion = (questionId) => {
    if (chapters.length === 0) {
      Alert.alert(
        ...
        "Veuillez d'abord créer des chapitres."
      );
      return;
    }
  ...
  const toggleAnswersDisplay = useCallback(
    async (questionId) => {
  ...
  const refreshPage = async () => {
    if (subjectActive) {
      ...
      Alert.alert(
        "Erreur",
        "Aucun sujet actif sélectionné. Veuillez sélectionner un sujet pour rafraîchir les données."
      );
    }
  };
  ...
  const handleSaving = async () => {
    ...
    try {
      ...
      if (errorUpdating) {
        ...
        const formattedContent = miscState.contentEdit
    ...
    if (Object.keys(activeQuestionAnswers)[0]) {
      const loadData = async () => {
        if (miscState.question && miscState.question.full_text) {
          if (
            miscState.question.full_text?.ops &&
            miscState.question.full_text.ops[0]
          ) {
            ...
            setMiscState((prevState) => ({
              ...prevState,
              contentRead: cleanReadHtml,
              contentEdit: cleanEditHtml,
              isLoading: false,
            }));
          }
        }
  ...
  return (
    <View
      style={[
        ...
        {
          width: "100%",
          paddingHorizontal: miscState.isLargeScreen ? "10" : "0",
        },
      ]}
    >
      <View>
        <Picker
          ...
          onValueChange={(itemValue, itemIndex) => {
            const selectedSubject = subjects.find(
              (subj) => subj.content_subject.id === itemValue
            ).content_subject;
        ...
        >
          {subjects.map((subj, index) => (
            <Picker.Item
              key={index}
              label={subj.content_subject.title}
              value={subj.content_subject.id}
            />
          ))}
        </Picker>
      ...
      */}
        ...
        <CarrousselOrientation
          ...
          setStatut={setStatut}
          statut={statut}
          accessRight={miscState.userStatus}
          subject={subject}
        />
      </View>
              ...
              <View style={styles.fullWidth}>
                <View style={globalStyles.container_wide}>
                  <Card style={globalStyles.QuestionBubble}>
                    <Card.Content>
                      <Paragraph style={globalStyles.globalButtonText_tag}>
                        Vous pouvez proposer ici des thèmes qui vous intéressent
                        et qui inspireront les contributeurs...
                      </Paragraph>
                    </Card.Content>
                  </Card>
            ...
            {vision === "contribution" && (
              <View style={styles.fullWidth}>
                {themesAllUsers.length === 0 ? (
                  ...
                  <View
                    style={[
                      globalStyles.container_wide,
                      {
                        width: miscState.isLargeScreen ? "90%" : "100%",
                        paddingHorizontal: miscState.isLargeScreen ? "10" : "0",
                      },
                    ]}
                  >
            ...
            {vision === "table" && statut != "Réagir" && (
              <View style={styles.fullWidth}>
                <View style={globalStyles.container_wide}>
                        ...
                        <TouchableOpacity
                          onPress={() => {
                            setMiscState((prevState) => ({
                              ...
                              toggleIcon: prevState.iconsVisible
                                ? plusIcon
                                : minusIcon,
                            }));
                          }}
                  ...
                  {statut === "Structurer" && subject.etape_collecting && (
                    <>
                      <p>
                        Voici la structure du projet. Les parties apparaissent
                        en <strong style={{ color: "black" }}>noir</strong> et
                        les chapitres en{" "}
                        <strong style={{ color: "blue" }}>bleu</strong> à
                        l'intérieur de chacune des parties.
                      </p>
                    </>
                  )}

                  {statut === "Publier" && subject.etape_publishing && (
                    <>
                      <p>
                        Les chapitres doivent être publiés individuellement.
                        Vous pouvez choisir une publication{" "}
                        <strong style={{ color: "#008080" }}>privée</strong> ou
                        une publication{" "}
                        <strong style={{ color: "#008080" }}>publique</strong>.{" "}
                      </p>
                      <p>
                        Dans le premier cas, seuls les presonnes ayant accès au
                        projet auront accès à la publication.{" "}
                      </p>
                      <p>
                        Dans le second cas, toute personne se rendant sur le
                        site internet bioScriptum y aura accès.
                      </p>
                    </>
                  )}
                </View>
                ...
                <ScrollView>
                  <View style={globalStyles.container_wide}>
                    {(miscState.hasUnclassifiedQuestions
                      ? [{ id: null, title: "Chapitres non classés" }]
                      : []
                    )
                      .concat(chapters)
                      .map((chapter) => (
                        <View key={chapter.id || "non-chapitre"}>
                          ...
                          >
                            ...
                            >
                              <Text style={globalStyles.title_chapter}>
                                {chapter.title}
                              </Text>
                            </TouchableOpacity>
                            {(miscState.iconsVisible ||
                              ...
                              <View style={{ flexDirection: "row" }}>
                                ...
                                <TouchableOpacity
                                  onPress={() =>
                                    confirmDeletion(chapter.id, true)
                                  }
                                  style={{ marginRight: 10 }}
                                >
                                  <Image
                          ...
                          {openChapters[chapter.id] && (
                            <>
                              {questions.filter(
                                ...
                                >
                                  Aucun chapitre trouvé dans cette partie
                                </Text>
                              ) : (
                                questions
                                  .filter((q) => q.id_chapitre === chapter.id)
                                  .map((question) => (
                                    <TouchableOpacity
                                      key={question.id}
                                      style={[
                                            ...
                                            >
                                              <TouchableOpacity
                                                onPress={() => {
                                                  ...
                                                  setModals((prevState) => ({
                                                    ...prevState,
                                                    isEditChapterModalVisible: true,
                                                  }));
                                                }}
                                                style={{ marginRight: 10 }}
                                              >
                                        ...
                                        {statut === "Publier" &&
                                          subject.etape_publishing && (
                                            ...
                                            >
                                              ...
                                              <ToggleButton
                                                bool={
                                                  question.published_private
                                                }
                                                setBool={() =>
                                                  handlePublication_private(
                                                    question.id,
                                                    !question.published_private
                                                  )
                                                }
                  ...
                  >
                    {statut === "Structurer" && subject.etape_writting && (
                      <>
                        <TouchableOpacity
                        ...
                        <TouchableOpacity
                          ...
                          onPress={() =>
                            setModals((prevState) => ({
                              ...prevState,
                              isModalNewQuestionVisible: true,
                            }))
                          }
                        >
                          <Text style={globalStyles.globalButtonText}>
  ...
  */}
          ...
          <ModalComponent
            ...
            inputValue={
              newTitles.newChapterTitle !== undefined
                ? newTitles.newChapterTitle
                : ""
            } // Correction ici
            onInputChange={(text) =>
              setNewTitles((prevState) => ({
          ...
          <ModalComponent
            ...
            inputValue={
              newTitles.newQuestionTitle !== undefined
                ? newTitles.newQuestionTitle
                : ""
            } // Correction ici
            onInputChange={(text) =>
              setNewTitles((prevState) => ({
                ...prevState,
                newQuestionTitle: text,
              }))
            }
            onSave={async () => {
              const question = await save_question(
                newTitles.newQuestionTitle,
                tags,
                subjectActive,
                setMiscState
          ...
          <ModalComponent
            ...
            inputValue={
              newTitles.editChapterTitle !== undefined
                ? newTitles.editChapterTitle
                : ""
            }
            onInputChange={(text) => {
              setNewTitles((prevState) => ({
          ...
          <ModalComponent
            ...
            inputValue={
              newTitles.editQuestionTitle !== undefined
                ? newTitles.editQuestionTitle
                : ""
            }
            onInputChange={(text) => {
              setNewTitles((prevState) => ({
                ...prevState,
                editQuestionTitle: text,
              }));
            }}
            onSave={async () => {
              await edit_question(
                selected.editQuestionId,
                newTitles.editQuestionTitle
              );
              setModals((prevState) => ({
                ...prevState,
          ...
          <ModalComponent
            ...
            title={`Êtes-vous sûr de vouloir supprimer ${
              selected.deletionDetails.isChapter
                ? "cette partie "
                : "ce chapitre "
