...
export default function InvitationScreen() {
  ...
  const { id_invitation } = route.params;
  ...
  useFocusEffect(
    React.useCallback(() => {
        getMemories_Question_by_id(id_invitation, setQuestion, setAnswers, setOwner);
    }, [id_invitation])
  );
  ...
  const refreshAnswers = async () => {
    await getMemories_Question_by_id(id_invitation, setQuestion, setAnswers, setOwner);
  };
    ...
    <View style={{ flex: 1, backgroundColor: "#E8FFF6" }}>
      <ScrollView contentContainerStyle={styles.contentContainer}>
        <View style={styles.navigationContainer}>
          ...
          <TouchableOpacity onPress={() => window.location.href = '/'} style={styles.navButton}>
            <Image source={logOut} style={{ width: 60, height: 60, opacity: 0.5 }} />
          </TouchableOpacity>
