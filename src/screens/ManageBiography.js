...
export default function ProjectScreen({ route }) {
  ...
  const [showContent, setShowContent] = useState("projets");
  ...
  const toggleLink = async (index, column) => {
    ...
    const updatedLinks = links.map(
      (link, idx) => (idx === index ? { ...link, [column]: newValue } : link) // Utilisation correcte de la propriété dynamique
    );
    ...
    <View style={globalStyles.container}>
      ...
      >
        {subject_active && (
          <TouchableOpacity
            ...
            style={[
              ...
              isHovered && globalStyles.navButton_over,
            ]}
        ...
        <TouchableOpacity
          ...
          style={[
            globalStyles.navButton,
            isHoveredPerson && globalStyles.navButton_over,
          ]}
      ...
      >
        {subjects_active.length > 0 && (
          <TouchableOpacity
            ...
            onPress={() => {
              setShowContent("projets");
            }}
          >
            <Text style={globalStyles.globalButtonText}>
              {showContent === "projets" &&
              subject_active &&
              subject_active.title
                ? "Changer le projet"
        ...
        {subjects_pending.length > 0 && (
          <TouchableOpacity
            ...
            onPress={() => {
              setShowContent("projets_pending");
            }}
          >
            <Text style={globalStyles.globalButtonText}>
              Mes projets en cours de validation{" "}
            </Text>
          </TouchableOpacity>
        )}
        <TouchableOpacity
                  ...
                  >
                    <Text
                      style={[
                        ...
                        subject_active?.id === subject.content_subject.id
                          ? globalStyles.globalButtonText_active
                          : globalStyles.buttonText,
                        {
                          textAlign: "left",
                          flex: 1,
          ...
          {subjects_pending.map((subject) => (
            ...
            >
              ...
              >
                ...
                <Text style={[globalStyles.buttonText, { textAlign: "right" }]}>
        ...
        {showContent === "projets_all" && (
          <View style={globalStyles.container_wide}>
            <View style={globalStyles.searchContainer}>
              ...
              <TouchableOpacity
                onPress={() =>
                  get_project(searchName, setLoading, setSearchResults)
                }
                style={styles.icon}
              >
                <Image
            ...
            {searchResults
              .filter(
                (result) =>
                  !subjects_active.some(
                    (subject) => subject.content_subject.id === result.id
                  )
              )
              .map((result) => (
                <TouchableOpacity
                ...
                >
                  <Text
                    style={[
                      globalStyles.globalButtonText_tag,
                      { color: "black" },
                    ]}
                  >
                    {result.title}
          ...
          <View style={globalStyles.container_wide}>
            <View style={globalStyles.searchContainer}>
              ...
              >
                <Image
                  source={AddIcon}
                  style={{ width: 26, height: 26, opacity: 0.5 }}
                />
              </TouchableOpacity>
            </View>
      ...
      {subject_active && subject_active.title && (
        <>
          <Text></Text>
          <Text style={globalStyles.title}>
            Gestion du projet actif : {subject_active.title}{" "}
          </Text>
          <Text></Text>
          ...
          {contributors?.length > 0 ? (
            <>
              ...
              {contributors.map((contributor) => (
                <View
                  key={contributor.id_user}
                  style={
                    isLargeScreen
                      ? styles.contributorRow
                      : styles.contributorColumn
                  }
                >
                  <Text style={styles.contributorText}>{contributor.name}</Text>
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Accès :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.access
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Inspirer :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Inspirer
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Raconter :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Raconter
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Réagir :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Réagir
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Structurer :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Structurer
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Rédiger :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Rédiger
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Relire :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Relire
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Publier :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Publier
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  {!isLargeScreen && (
                    <Text style={styles.labelText}>Lire :</Text>
                  )}
                  <Picker
                    selectedValue={
                      contributorStates[contributor.id_user]?.Lire
                        ? "Oui"
                        : "Non"
                    }
                    onValueChange={async (itemValue) => {
                  ...
                  <TouchableOpacity
                    onPress={async () => {
                      await deleteExistingContributor(
                        contributor.id_user,
                        subject_active.id
                      );
                      fetchContributors();
      ...
      {showContent === "liens" && (
        <View style={globalStyles.container_wide}>
                ...
                <View key={link.id} style={styles.linkRow}>
                  ...
                  <View style={styles.toggleContainer}>
                    <TouchableOpacity
                      style={[
                        styles.toggleButton,
                        !link.expired && styles.activeToggle,
                      ]}
                    ...
                    >
                      <View
                        style={[
                          ...
                          {
                            left: link.expired ? 2 : null,
                            right: link.expired ? null : 2,
                          },
                        ]}
                      />
                    </TouchableOpacity>
                      ...
                      {[
                        ...
                        <View key={field} style={styles.toggleContainer}>
                          <TouchableOpacity
                            style={[
                              styles.toggleButton,
                              link[field] && styles.activeToggle,
                            ]}
                          ...
                          >
                            <View
                              style={[
                                styles.toggleButtonCircle,
                                {
                                  left: link[field] ? null : 2,
                                  right: link[field] ? 2 : null,
                                },
                              ]}
                            />
                          </TouchableOpacity>
                  ...
                  <View style={styles.toggleContainer}>
                    <TouchableOpacity
                      onPress={() =>
                        copyLinkToClipboard(
                          `https://bioscriptum.com/marcel/${link.id}`
                        )
                      }
                    >
          ...
          <TouchableOpacity
            style={styles.createLinkButtonContainer}
            onPress={async () => {
          ...
          <View style={styles.modalView}>
            <TouchableOpacity
              ...
              onPress={async () => {
                ...
                setJoinMessage(
                  "Votre demande a bien été envoyée aux contributeurs du projet pour validation."
                );
              }}
            >
              <Text style={[globalStyles.globalButtonText]}>
                Envoyer la demande d'accès au projet
              </Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[globalStyles.globalButton_wide]}
