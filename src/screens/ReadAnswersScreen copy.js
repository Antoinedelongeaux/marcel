...
function ReadQuestionsScreen({ route }) {
    ...
    return ReactHtmlParser(content, {
      transform: (node, index) => {
        if (node.type === 'tag' && node.name === 'reference') {
          return (
            <Pressable key={index} onPress={() => onReferencePress(node.children[0].data)} style={{ pointerEvents: 'auto' }}>
  ...
  const RenderContent = memo(({ content, onReferencePress }) => {
    ...
    const transformNode = useCallback((node, index) => {
      if (node.type === 'tag' && node.name === 'reference') {
        return (
          <TouchableOpacity key={index} onPress={() => onReferencePress(node.children[0].data)} style={[styles.associateButton, { pointerEvents: 'auto' }]}>
            <Text style={{ color: 'blue' }}><Image source={LinkIcon} style={{ width: 18, height: 18, opacity: 0.5 }} /></Text>
          </TouchableOpacity>
        );
      }
  ...
  useEffect(() => {
    console.log("State 'question' has changed:", question);
  }, [question]);
   ...
   /*
          ...
          */
            ...
            if (userStatus.chapters === "Editeur"){
              ...
              console.log("html : ",html)
              const html_clean = html.replace(/<p>(.*?)&lt;reference&gt;/, '<p>$1</p>&lt;reference&gt;<p>').replace(/&lt;&#x2F;reference&gt;(.*?)<\/p>/, '</p>&lt;&#x2F;reference&gt;<p>$1</p>');
              console.log("html : ",html_clean)
              setContent(decode(html_clean));
            }
    ...
    <>
      {question && question.question && (
         ...
         <> 
          ...
          <RenderContent content={content} onReferencePress={handleReferencePress} />
          </>
      )}
...
const styles = StyleSheet.create({
  ...
  contentContainer: {
    zIndex: 10, // Cette valeur assure que le contenu est toujours en premier plan
    position: 'relative',
  },
});
